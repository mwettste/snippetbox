...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Short password", "Bob", "bob@example.com", "pa$$word", csrfToken, http.StatusOK, []byte("This field is too short (min. 10 characters)")},
...
func TestCreateSnippetForm(t *testing.T) {
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		if actualLocation != expectedLocation {
			t.Errorf("expected %s; got %s", expectedLocation, actualLocation)
		}
	})
